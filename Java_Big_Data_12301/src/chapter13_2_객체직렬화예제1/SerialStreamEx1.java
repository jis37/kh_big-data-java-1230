...
// 직렬화 : 프로그램에서 사용되는 개체를 파일이나 네트워크로 전송하기 위해서 바이트 형태로 변경하는 것
...
public class SerialStreamEx1 {
    ...
    static List<Board> readList() throws IOException, ClassNotFoundException {
        ...
        List<Board> list = (List<Board>) ois.readObject(); // 파일의 바이너리를 역직렬화(원하는 타입으로 변환함) // ***
        return list;
    }
}
