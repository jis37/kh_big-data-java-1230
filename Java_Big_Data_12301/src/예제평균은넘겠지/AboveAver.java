...
public class AboveAver {
    public static void main(String[] args) {
        ...
        double[] rst = new double[testCnt]; // 소수점 결과 값에 대한 배열
    ...
    // 구한 평균을 가지고 각각의 입력 받은 성적을 비교해 평균이 넘는 학생수를 구함
    // 구한 학생수를 백분율로 변환해서 반환
    static double overRate(){ // 반환 타입이 double -> 소수점까지 나타내려함
        Scanner sc = new Scanner(System.in);
        int cnt = sc.nextInt(); // 학생 수 입력
        int total = 0; // 총점을 구하기 위한 변수
        int overCnt = 0; // 평균을 넘는 학생 수를 구하는 변수
        int[] score = new int[cnt]; // 학생의 수 만큼 성적을 입력 받기 위한 배열 생성
        for(int i = 0; i < score.length;i++){
            score[i] = sc.nextInt(); // 학생의 수 만큼 성적을 입력 받아 배열에 대입
            total += score[i]; // 학생의 수 만큼 입력된 성적을 누적해서 총점을 구함
        }
        double aver = (double)total / cnt; // 총점을 학생의 수로 나누어 평균을 구함
        for(int e : score){ // e -> 배열에 포함되어 있는 각각의 학생 성적 (e는 0번부터 score에 있는 인덱스를 순회함)
