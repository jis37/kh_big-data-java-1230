...
class ProductTV extends ProtoTypeTV{
    ...
    @Override // 접근제한자까지 모두 동일해야 오버라이딩 모드가 활성화 됨
    public void setChannel(int cnl){
        if(cnl > 0 && cnl < 2000) {
            channel = cnl;
        } else {
    ...
    public void setChannel(int cnl, boolean isInternet){
        if(isInternet){
            System.out.println("인너넷 모드 입니다.");
            this.isInternet = true; // 인터넷 모드 사용시 채널이 필요 없기 때문에 채널 적용이 안됨
        } else{
            this.isInternet = false;
//            if(cnl > 0 && cnl < 2000) { // 강사님이 적용한 코드
//                channel = cnl;
//            } else {
//                System.out.println("채널 설정 범위를 벗어났습니다.");
//            }
        }
        if(cnl > 0 && cnl < 2000) { // 이중 if로 사용하면 채널 표기는 가능 함
            channel = cnl;
        } else {
            System.out.println("채널 설정 범위를 벗어났습니다.");
        }
