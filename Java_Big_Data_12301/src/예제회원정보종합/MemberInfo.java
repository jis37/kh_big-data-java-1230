package 예제회원정보종합;
// 이름, 나이, 성별, 직업을 입력 받아 출력하기

import java.util.Scanner;

public class MemberInfo { // 필드와 메소드가 있음
    // 반복문을 제어 하는 명령문 : break(수행 후 조건에 걸리면 멈춤, continue(반복문의 조건(처음)으로 되돌아감)
    ...
    public void viewInfo(){
        ...
        System.out.println("성별 : " + genderStr[getGenderType()]); // 위 추가된 예시 메소드 타입을 정수로 바꾸고 문자 배열을 만든 코드에 대입 / 첫 배열은 0번으로 빈칸으로 둠
        System.out.println("직업 : " + jobsStr[jobs]); // 위 문자 배열에서 첫번째 순서 0을 빈칸으로 두고 1,2,3,4 순으로 출력 됨
    }
}
