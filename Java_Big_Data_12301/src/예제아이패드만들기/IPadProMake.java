...
public class IPadProMake {
    ...
    IPadProMake(String name){ // 생성자
        this.name = name;
        Date now = new Date(); // 현재의 시간 정보를 가져오기 위해 사용 (Date 클래스)
        SimpleDateFormat sdf = new SimpleDateFormat("yyMMdd");
        productDate = sdf.format(now); // 시간 정보 중 "yyMMdd" 패턴으로 시간 정보를 가져옴
        cnt++; // 생성자는 객테가 만들어 질 때 호출 되므로 생성된 객체의 횟수를 확인하는 용도로 사용 할 수 있음
    ...
    boolean continueOrder(){
        ...
        System.out.print("구입 하실려면 yes / 종료는 quit : ");
        String isContinue = sc.next();
        if (isContinue.equalsIgnoreCase("yes")) return true; // 시험 문제(문자열 비교 연산자는 ( == ) 가 아닌 equals로 사용 Ignore -> 대소문자 구별 없음
        return false;
    }
    // 화면 크기 정하기
    void setScreen(){ // 내부에서 값을 저장하는 void
        Scanner sc = new Scanner(System.in);
        while(true){
    ...
    // 일련 번호 만들기 : iPad + 11/13 + 128/256/512/1024 + W/C + 230112 + 생산 댓수
    void setSerialNum(){
    ...
    void inProductPad() throws InterruptedException {
        int cnt = 0;
        while(true){
            sleep(300); // 0.3초 동안 슬립 모드( 코드 2개 추가되어야 함 클릭 후 추가 )
            cnt++;
            System.out.print(" << iPad Pro 제작 중 : [" + cnt + "%] >> "); //cnt 만 바꿀려면 아래 코드를 추가
            System.out.print("\r"); // 캐리지?리턴 커서를 앞으로 이동
            if(cnt >= 100) break;
        }
    }
