package chapter6_오버라이딩제약;

public class Vehicle {
    protected int speed; //protected 는 상속관계와 동일 패키지 내에서 접근 가능
    protected String color;
    protected String name;
    // final 은 최종 상태를 의미하며, 메소드에서 사용 시 오버로딩이 불가함을 의미 합니다.
...
class SportsCar extends Vehicle{
    ...
    void setTurbo(boolean x){ // x -> 이름은 상관 없이 값이 중요함
        isTurbo = x;
    }
    void infoCar(){
        System.out.println("이름 : " + name);
        System.out.println("속도 : " + getSpeed()); // 메소드 안에서 메소드를 불러옴
